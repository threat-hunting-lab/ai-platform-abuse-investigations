...
CASE_0005_REQUIRED = ["model_repo_access", "file_transfers", "auth_sessions"]
...
def export_query_to_csv(
    ...
    Substitutes ${DATA} with the actual data directory path before executing.
    ...
    # Substitute ${DATA} placeholder with actual data directory path
    if data_dir:
...
def main() -> None:
    ...
    ap.add_argument("--sql", required=True, help="Directory containing SQL files for this case")
    ap.add_argument("--case-dir", required=True, help="Case study directory (e.g., case_studies/CASE-0001-...)")
    ap.add_argument("--strict", action="store_true", help="Fail if required parquet tables are missing")
    args = ap.parse_args()
    ...
    sql_dir_lower = str(sql_dir).lower()
    if "case0005" in sql_dir_lower:
        required_tables = CASE_0005_REQUIRED
    elif "case0002" in sql_dir_lower:
        required_tables = CASE_0002_REQUIRED
    elif "case0003" in sql_dir_lower:
        required_tables = []
    elif "case0004" in sql_dir_lower:
        required_tables = []
    else:
    ...
    # Create helpful empty views if optional tables absent
    ...
    for sql_path in sql_files:
        ...
        try:
            # Pass data_dir so ${DATA} gets substituted inside the function
            row_count, cols = export_query_to_csv(con, sql_path, out_csv, data_dir)
            artifacts.append({
